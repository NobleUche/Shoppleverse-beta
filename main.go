...
func main() {
	...
	log.Fatal(http.ListenAndServe(":8000",
		handlers.CORS(
			handlers.AllowedOrigins([]strings{"*"}),
			handlers.AllowedMethods([]strings{"GET", "POST", "PUT", "DELETE"}),
			handlers.AllowedHeaders([]strings{"X-Requested-With", "Content-Type", "Authorization"}),)(router)))
}
